...
from .core.config import ConfigError, create_example_config, load_config
from .core.scraper import scrape_version_number
from .utils.auto_installer import AutoInstaller
...
@click.option(
    "--create-example", is_flag=True, help="Create example configuration file"
)
...
def check(
    ...
    if create_example:
        ...
        click.echo("Created config.yaml.example")
        return
    ...
    try:
        ...
        if result:
            if config_data.get("detailed_info", False):
                ...
                if result["needsUpdate"]:
                    ...
                    # Handle auto-install if flag is set
                    if auto_install:
                        ...
                        if is_fresh_install:
                            step_descriptions = [
                                ...
                                "Extract and install to the configured location",
                                "Start the application",
                                "Clean up temporary files",
                            ]
                        else:
                            step_descriptions = [
                                "Stop any running OlivedPro processes",
        ...
        if "not found" in str(e).lower():
            click.echo(
                "\nRun 'version-checker --create-example' to create a sample configuration file."
            )
...
def show_config(path: bool, config: str | None, raw: bool) -> None:
    ...
    if path:
        ...
        if config_file.exists():
            ...
            console.print(
                "\n[yellow]Run 'version-checker check --create-example' to create an example config.[/yellow]"
            )
        return
    ...
    if not config_file.exists():
        console.print(
            f"[red]Configuration file not found:[/red] {config_file}",
            style="bold",
        )
        console.print(
            "\n[yellow]Run 'version-checker check --create-example' to create an example config.[/yellow]"
